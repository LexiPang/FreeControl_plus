...
from libs.model.module.scheduler import CustomDDIMScheduler, CustomEulerDiscreteScheduler
from diffusers import StableDiffusionPipeline, DDPMScheduler, EulerDiscreteScheduler
from diffusion_restart_sampling.diffuser.diffusers.schedulers.scheduling_ddim import DDIMScheduler
#from diffusers import SDEScheduler
...
def freecontrol_generate(condition_image, prompt, scale, ddim_steps, sd_version,
                         ...
                         pca_basis_dropdown, inversion_prompt, condition, img_size, restart, second_order,
                         restart_list, hide_verbose, visualize, save_interval, **kwargs):
    ...
    # pipeline.scheduler = CustomEulerDiscreteScheduler.from_config(pipeline.scheduler.config)
    # 应该不能用EDM Scheduler，因为去噪过程的timesteps与DDIM Inversion的timesteps对不上
    pipeline.scheduler.use_karras_sigmas = False
    ...
    pipeline_output, denoising_img_list = pipeline(prompt=config.sd_config.prompt,
    ...
    img_list = pipeline_output.images
    # Display the result：
    ...
    return img_list, denoising_img_list
...
def main():
    ...
    block.launch(server_name='0.0.0.0', share=False, server_port=9990)
