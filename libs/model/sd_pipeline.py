...
# from diffusion_restart_sampling.diffuser.diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion import StableDiffusionPipeline
...
class FreeControlSDPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        guidance_scale = self.input_config.sd_config.guidance_scale
        ...
        # check and see if the reference object can be found in the inversion prompt, else raise error
        # through comparing the ref token with each of the tokens in the inversion prompt
            try:
        ...
        # check and see if the generated object can be found in the generation prompt, else raise error
        # through comparing the gen token with each of the tokens in the generation prompt
        ...
        # prompt is prompt used for generation (such as "a photo of a lion ...")
        if prompt is not None and isinstance(prompt, str):
            batch_size = 1
        # if prompt is str, only one image is generated
        elif prompt is not None and isinstance(prompt, list):
            batch_size = len(prompt)
        # if prompt is list (many sentences), batch_size=len(prompt) images are generated
        else:
            batch_size = prompt_embeds.shape[0]
        # no prompt provided, only prompt embeds
        ...
        # randomly intialize the latents, then scale the noise according to the requirements of the scheduler
        if same_latent:
            keep_latents = latents
        latents = torch.cat([latents] * 2, dim=0)
        # one for unconditional (negative) and one for conditional
        '''
        ...
        # intialize the number of appearance samples to equal the number of control samples
        total_samples: int = 0
        if config.data.inversion.method == 'DDIM':
            uncond_example_ids: List[int] = list()
            # empty list because there are no unconditional example samples
        ...
        # "+" means shifting the range by (num_control_samples * 2 + len(uncond_example_ids)), this is the number of 
        # samples before conditional example samples
        cond_control_ids: List[int] = (np.arange(0, num_control_samples, 1) + (cond_example_ids[-1] + 1)).tolist()
        # shift by (cond_example_ids[-1] + 1), logic same as above
        ...
        # shift by (cond_control_ids[-1] + 1), logic same as above
        ...
        # keep_ids holds the ids of samples that are not example samples
        ...
        if restart:
            ...
            # all_sigmas represents the standard deviation of the noise at each timestep, calculated from cumulative product of alphas.
            # all_sigmas[timesteps.cpu().numpy()] selects specific sigma values for the current set of timesteps,
            # the timesteps are indices that indicate which parts of the diffusion process the model is currently in.
            ...
            # print(max(sigmas))
            # print(min(sigmas))
            # the original key in restart_list is also a noise level (sigma)
            # this code finds the index of the sigma value in sigmas that is closest to the key.
        ...
        image_list = []
        with self.progress_bar(total=num_inference_steps, verbose=verbose) as progress_bar:
            for i, t in enumerate(timesteps[:-1]):
                if verbose:
                    print(f"index:{i}, timestep:{t}")
                score = None
                ...
                # inverted_data['condition_input'] == condition_image_latents (pipeline.invert(img, inversion_config))

                if config.data.inversion.method == 'DDIM':
                    if i == 0 and same_latent and config.sd_config.appearnace_same_latent:
                        latents = data_samples_latent.repeat(2, 1, 1, 1)
                        print("case 1 is triggered")
                ...
                if config.data.inversion.method == 'DDIM':
                    ...
                    # the negative prompt embeds and prompt embeds are concatenated together, so we need to
                    # separate and rearrange them first before passing them to the unet
                else:
                    raise NotImplementedError("Currently only support DDIM method")
                ...
                latents_next = self.scheduler.step(noise_pred, i, latents, score=score,
                ...
                # latents_next = self.scheduler.step(noise_pred, i, latents, **extra_step_kwargs)[0].detach()
                # 这里的latents与Restart SD里的latents不一样在于这里已经做了*2操作再传入.step的，应该就是同时采样两个图像，一个有structural guidance一个没有
                
                # Apply 2nd order correction (Heun).
                if second_order and i < len(timesteps) - 2:
                    if verbose:
                        print("Applying 2nd order correction")
                    t_next = timesteps[i + 1]
                
                    assert do_classifier_free_guidance, "Currently only support classifier free guidance"
                        # Process the latent
                    step_timestep: int = t_next.detach().cpu().item()
                    assert step_timestep in inverted_data['condition_input'][0][
                        'all_latents'].keys(), f"timestep {step_timestep} not in inverse samples keys"
                    data_samples_latent: torch.Tensor = inverted_data['condition_input'][0]['all_latents'][step_timestep]
                    data_samples_latent = data_samples_latent.to(device=self.running_device, dtype=prompt_embeds.dtype)
                    ...
                    if config.data.inversion.method == 'DDIM':
                        if i == 0 and same_latent and config.sd_config.appearnace_same_latent:
                            ...
                            latents = torch.cat([data_samples_latent, keep_latents], dim=0)
                            print("Latents shape", latents.shape)
                            latent_list: List[torch.Tensor] = [latents_next, data_samples_latent, latents_next]
                    else:
                        raise NotImplementedError("Currently only support DDIM method")

                    latent_model_input: torch.Tensor = torch.cat(latent_list, dim=0).to('cuda')
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t_next).detach()
                    ...
                    # perform guidance
                    if do_classifier_free_guidance:
                        noise_pred_prime = noise_pred_prime[keep_ids]
                        noise_pred_uncond_prime, noise_pred_text_prime = noise_pred_prime.chunk(2)
                        noise_pred_prime = noise_pred_uncond_prime + guidance_scale * (noise_pred_text_prime - noise_pred_uncond_prime)
                                    
                    if do_classifier_free_guidance and guidance_rescale > 0.0:
                        # Based on 3.4. in https://arxiv.org/pdf/2305.08891.pdf
                        noise_pred_prime = rescale_noise_cfg(noise_pred_prime, noise_pred_text_prime, guidance_rescale=guidance_rescale)

                    latents = self.scheduler.step(0.5 * noise_pred + 0.5 * noise_pred_prime, i, latents, score=score,
                        guidance_scale=self.input_config.sd_config.grad_guidance_scale,
                        indices=temp_control_ids,
                        **extra_step_kwargs, return_dict=False)[0].detach()
                else:
                ...
                if visualize and i % save_interval == 0:
                    ...
                    cond_image = self.decode_latents(latents[0].unsqueeze(0))
                    uncond_image = self.decode_latents(latents[1].unsqueeze(0))
                    # image_list.append((t,cond_image))
                    image_list.append((t,(cond_image, uncond_image)))
                ...
                if i + 1 in restart_list.keys():
                    ...
                    for restart_iter in range(int(restart_list[restart_idx][1])):
                        ...
                        new_scheduler.timesteps = torch.round(torch.linspace(timesteps[i+1], max_idx, int(restart_list[restart_idx][0])).flip([0])).long()
                        # new_scheduler.timesteps = torch.round(torch.arange(timesteps[i+1], max_idx, 5).flip([0])).long()
                        # new_scheduler.timesteps = torch.round(torch.arange(timesteps[i+1], restart_list[restart_idx][2], 5).flip([0])).long()
                        ...
                        for j, new_t in enumerate(new_t_steps[:-1]):
                            ...
                            if config.data.inversion.method == 'DDIM':
                                if j == 0 and same_latent and config.sd_config.appearnace_same_latent:
                                    latents = data_samples_latent.repeat(2, 1, 1, 1)
                                    print("case 1 triggered!")
                                elif j == 0 and same_latent and not config.sd_config.appearnace_same_latent:
                                    latents = torch.cat([data_samples_latent, keep_latents], dim=0)
                            ...
                            latent_model_input: torch.Tensor = torch.cat(latent_list, dim=0).to('cuda')
                            latent_model_input = new_scheduler.scale_model_input(latent_model_input, new_t).detach()
                            
                            # process the prompt embedding
                            if config.data.inversion.method == 'DDIM':
                                ref_prompt_embeds = inverted_data['condition_input'][0]['prompt_embeds'].to('cuda')
                                # prompt_embeds: an embedding of the inversion prompt
                                step_prompt_embeds_list: List[torch.Tensor] = [prompt_embeds.chunk(2)[0]] * 2 + [
                                    ref_prompt_embeds] + [prompt_embeds.chunk(2)[1]] * 2
                            else:
                                raise NotImplementedError("Currently only support DDIM method")
                            ...
                            require_grad_flag = False
                            # Check if the current step is in the guidance step
                            if _in_step(self.guidance_config.pca_guidance, j):
                                require_grad_flag = True
                            ...
                            loss = 0
                            self.cross_seg = None
                            if _in_step(self.guidance_config.cross_attn, j):
                                # Compute the Cross-Attention loss and update the cross attention mask, Please don't delete this
                                self.compute_cross_attn_mask(cond_control_ids, cond_example_ids, cond_appearance_ids)
                            ...
                            if isinstance(loss, torch.Tensor):
                                gradient = torch.autograd.grad(loss, latent_model_input, allow_unused=True)[0]
                                gradient = gradient[cond_control_ids]
                                assert gradient is not None, f"Step {j}: grad is None"
                                score = gradient.detach()
                                temp_control_ids: List[int] = np.arange(num_control_samples).tolist()
                            ...
                            # compute the previous noisy sample x_t -> x_t-1
                            latents_next = new_scheduler.step(noise_pred, j, latents, score=score,
                                guidance_scale=self.input_config.sd_config.grad_guidance_scale,
                                indices=temp_control_ids,
                                **extra_step_kwargs, return_dict=False)[0].detach()
                            
                            # latents_next = new_scheduler.step(noise_pred, j, latents, **extra_step_kwargs)[0].detach()
                            ...
                            if  (j < len(new_t_steps) - 2 or new_t_steps[-1] > 1):
                                ...
                                assert do_classifier_free_guidance, "Currently only support classifier free guidance"
                                # Process the latent
                                step_timestep: int = new_t_next.detach().cpu().item()
                                assert step_timestep in inverted_data['condition_input'][0][
                                    'all_latents'].keys(), f"timestep {step_timestep} not in inverse samples keys"
                                data_samples_latent: torch.Tensor = inverted_data['condition_input'][0]['all_latents'][step_timestep]
                                data_samples_latent = data_samples_latent.to(device=self.running_device, dtype=prompt_embeds.dtype)
                                ...
                                latent_model_input: torch.Tensor = torch.cat(latent_list, dim=0).to('cuda')
                                latent_model_input = new_scheduler.scale_model_input(latent_model_input, new_t_next).detach()
                                ...
                                latents = new_scheduler.step(0.5 * noise_pred + 0.5 * noise_pred_prime, j, latents, score=score,
                                    guidance_scale=self.input_config.sd_config.grad_guidance_scale,
                                    indices=temp_control_ids,
                                    **extra_step_kwargs, return_dict=False)[0].detach()
                                
                                # latents = new_scheduler.step(0.5 * noise_pred + 0.5 * noise_pred_prime, j, latents, **extra_step_kwargs)[0].detach()
                            else:
                                latents = latents_next
                            ...
                            if visualize:
                                ...
                                # image = self.image_processor.postprocess(image, output_type=output_type, do_denormalize=do_denormalize)
                                # image_list.append(image)
                                # image_list.append((new_t,self.decode_latents(latents[0].unsqueeze(0))))
                                cond_image = self.decode_latents(latents[0].unsqueeze(0))
                                uncond_image = self.decode_latents(latents[1].unsqueeze(0))
                                image_list.append((new_t,(cond_image, uncond_image)))
    ...
    def ddim_inversion(
        ...
        # encode image with vae, transform image to latents
